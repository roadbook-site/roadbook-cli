...
- **DO** use the `rb.radar` utility to scan and understand the page structure quickly during exploration (e.g., `rb.radar.scan_interactive_elements()`, `rb.radar.scan_tables()`) before blindly guessing DOM selectors. This reduces trial-and-error.
...
- **DO** strictly use `rb.crossroads()`, `rb.pause_for_manual_action()` or `rb.wait_for_condition()` when encountering ambiguous decisions, login structures, or CAPTCHAs. Rely on the SDK for state management.
...
- **DON'T** attempt to bypass or brute-force Login screens or write automated password inputs. Always use `rb.pause_for_manual_action()` or `rb.wait_for_condition()`.
- **DON'T** run scripts that require manual intervention silently in the background; always prompt the user to execute them in their visible terminal.
...
1.  **Micro-Testing & Reconnaissance**: Open `scripts/tests/sample_test.py` and read the instructions. Organize your exploration by **Sheet** (One Sheet, One Scene). Write minimal tests for complex interactions here first before merging into `script.py`. 
2.  **DOM Scanning & Diffing**: Use `rb.radar.scan_all()` or specific radar methods to map the DOM. If you are unsure what changes after a click or login, use `rb.auth.snapshot_state("pre_action")`, perform the action, take a `post_action` snapshot, and use `rb.auth.diff_states()` to see exactly what elements appeared or disappeared. This removes guesswork.
3.  **Develop Workflow**: Read and modify `scripts/script.py`. 
    ...
    - **Anti-bot & Login**: `roadbook init` does not handle advanced anti-bot measures. When exploring login processes, leverage `rb.auth.ensure_login()` with the appropriate `AuthMode` (e.g., `ANTI_BOT_INTERACTIVE`). Avoid brute-forcing or typing passwords automatically.
    - **User Intervention & Agent Handoff**: When using `rb.wait_for_condition()`, try to provide a `success_selector` (like a dashboard avatar) so it can auto-resume. If impossible, use `rb.pause_for_manual_action()` and remind the user to interact via their terminal. If you need the Agent to make an ambiguous decision during execution, use `rb.crossroads("question")` to pause and await a reply.
    - Follow the inline `[AGENT INSTRUCTION]` blocks for artifact handling. Use `rb.sheet` to map your logic to sheets.
4.  **Programmatic Output (Script Sheet)**: Output structured data (JSON/dict) representing the discovered workflow steps and outputs using `rb.emit_output()` (only final data matching `TaskOutput`).
5.  **Verify & Sync**: Run the script to verify. Clean up dead ends and isolate the successful path.
