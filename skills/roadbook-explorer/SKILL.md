...
1.  **Micro-Testing & Reconnaissance**: Open `scripts/tests/sample_test.py` and read the instructions. Write minimal tests for complex interactions here first. Use `rb.radar.scan_all()` or specific radar methods to map the DOM and extract relevant targets (buttons, links, tables) instead of blind trial-and-error.
