...
3.  **Develop Workflow**: Read and modify `scripts/script.py`. 
    ...
    - **User Intervention**: When using `rb.wait_for_human_action()`, try to provide a `success_selector` (like a dashboard avatar) so it can auto-resume. If impossible, ensure `wait_for_user_input=True` is set and remind the user to interact via their terminal.
