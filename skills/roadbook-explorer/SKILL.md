...
- **DO** strictly use `rb.pause_for_manual_action()` or `rb.wait_for_condition()` when encountering login structures or CAPTCHAs. Rely on the SDK for state management.
...
3.  **Develop Workflow**: Read and modify `scripts/script.py`. 
    ...
    - **User Intervention**: When using `rb.wait_for_condition()`, try to provide a `success_selector` (like a dashboard avatar) so it can auto-resume. If impossible, use `rb.pause_for_manual_action()` and remind the user to interact via their terminal.
