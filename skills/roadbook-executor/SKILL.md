...
4.  **Robust Execution Guidelines**:
    ...
    - **Authentication Handling**: If a roadbook has `requires_login: true` in its constraints, ensure that the execution environment has a valid `state.json` (usually managed by the SDK or initialized via `roadbook init --login y`). Do not attempt to write raw password-typing scripts; rely on the user-provided session state or prompt the user if the session is invalid.
