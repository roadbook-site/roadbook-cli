...
4.  **Robust Execution Guidelines**:
    - **Composability & Modules**: If a roadbook has `role: module` in its frontmatter, it is a reusable component designed to be called by other roadbooks via `rb.call_roadbook()`. You should not run it directly without `--mock-input`. Remind the user to provide mock inputs or find the parent roadbook.
    ...
    - **Site Constraints Handling**: Read the `**Constraints**:` block from `roadbook.md` (e.g., `requires_login`, `auto_solve_captcha`, `stealth_mode`, `viewport`) and ensure these are faithfully injected as a dictionary to the `site_overrides` parameter when instantiating `RoadbookContext` in `script.py`.
    - **Authentication Handling**: If a roadbook has `requires_login: true` in its constraints, ensure that the execution environment uses `rb.auth.ensure_login()` with the appropriate `AuthMode`. Do not attempt to write raw password-typing scripts; rely on the built-in auth manager or prompt the user (`rb.pause_for_manual_action`) if the session is invalid.
    - **DOM State Verification**: Use `rb.auth.snapshot_state()` and `rb.auth.diff_states()` if you need to strictly verify that an action (like a click) actually caused new elements to appear on the page, preventing false positives.
