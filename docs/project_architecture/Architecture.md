...
| **CLI (工具箱)** | **Gear (装备)** | 探险家的交互界面。除了执行命令，它更是一个**交互式向导**。报错不仅仅是 Error，而是 Actionable Advice（可执行建议）。 |
...
| 层面 | 核心组件 | 功能描述 |
| :--- | :--- | :--- |
| **Agent 交互层** | **API / CLI** | **Interactive Guide (交互向导)**。Agent 通过它不仅传达意图，还能接收环境诊断信息。CLI 需具备**引导性 (Guidance)**，在失败时提供具体修复步骤（如“请安装 playwright”或“将脚本保存至 X 路径”）。 |
